...
class SplitCrossEntropyLoss(nn.Module):
    ...
    def distance(self, hiddens, weight, bias):
        ...
        for i in range(hiddens.size(0)):
            dist = dist_fn(hiddens[i], weight) + bias
            distances.append(dist)

        distances = [self.apply_threshold(d,h.detach()) for d,h in zip(distances, hiddens)]
    ...
    def logprob(self, weight, bias, hiddens, splits=None, softmaxed_head_res=None, verbose=False):
        ...
        if softmaxed_head_res is None:
            ...
            head_res = self.distance(hiddens, head_weight, head_bias)
        ...
        for idx in splits:
            ...
            # If the target is in one of the splits, the probability is the p(tombstone) * p(word within tombstone)
                ...
                tail_res = self.distance(hiddens, tail_weight, tail_bias)
    ...
    def forward(self, weight, bias, hiddens, targets, training, apply_thresh='none', verbose=False):
        ...
        overall_entropy = softmaxed_all_head_res
        running_offset = 0
        for idx in range(self.nsplits):
        ...
        return (total_loss / len(targets)).type_as(weight), overall_entropy
