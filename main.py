...
parser.add_argument('--data', type=str, default='data/testlang/', #penn/',
...
def evaluate(data_source, epoch, batch_size=1):
    ...
    if epoch % 10 == 0:
        pass
        #dump(model.encoder.weight.detach().cpu().numpy(), 'words_' + str(epoch))
        #dump(torch.cat(entropies).cpu().detach().numpy(), 'entropy_' + str(epoch))
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if 't0' in optimizer.param_groups[0]:
            ...
            val_loss2 = evaluate(val_data, epoch, eval_batch_size)
            print('-' * 89)
...
# Run on test data.
test_loss = evaluate(test_data, epoch, test_batch_size)
print('=' * 89)
